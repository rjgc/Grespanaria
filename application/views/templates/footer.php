 <!-- FOOTER -->
    <footer>
            <div class="container">
                <div class="inner footer">
                    <p class="footer-title"><?= lang('g_grespanaria'); ?> <span>Portugal</span></p>
                    <div class="row">
                        <div class="col-md-12">
                            <img class="footer-img-portugal" src="<?php echo base_url() ?>assets/grespanaria/img/margres.png" width="82" height="19" title="MARGRES">
                            <img class="footer-img-portugal" src="<?php echo base_url() ?>assets/grespanaria/img/love.png" width="42" height="19" title="LOVE">
                            <img class="footer-img-portugal" src="<?php echo base_url() ?>assets/grespanaria/img/bloom.png" width="63" height="23" title="BLOOM">
                            <img class="footer-img-portugal" src="<?php echo base_url() ?>assets/grespanaria/img/easy.png" width="58" height="23" title="EASY">
                        </div>
                    </div>
                    <p class="footer-title">Panaria <span>Business World</span><img class="shape" src="<?php echo base_url() ?>assets/grespanaria/img/shape.png"></p>
                    <div class="row">
                        <div class="col-md-12">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/panaria2.png" width="107" height="24" title="PANARIA GROUP">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/panaria.png" width="38" height="39" title="PANARIA">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/lea.png" width="41" height="19" title="LEA">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/cottodeste.png" width="83" height="19" title="COTTODESTE">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/fiordo.png" width="68" height="19" title="FIORDO">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/floridatile.png" width="79" height="21" title="FLORIDATILE">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/blustyle.png" width="61" height="19" title="BLUSTYLE">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/margres.png" width="82" height="19" title="MARGRES">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/love.png" width="42" height="19" title="LOVE">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/bloom.png" width="63" height="23" title="BLOOM">
                            <img class="footer-img" src="<?php echo base_url() ?>assets/grespanaria/img/easy.png" width="58" height="23" title="EASY">
                        </div>
                    </div>
                    <p class="copyright">&copy; 2014 <span>Gres Panaria Portugal S.A.</span> | <a href="<?= site_url('pages/disclaimer'); ?>"><?= lang('g_disclaimer'); ?></a></p>
                </div>
            </div>
        </footer>
    </body>
</html>
